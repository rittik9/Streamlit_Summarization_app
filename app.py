...
def pre_process(df):
    ...
    df['content'] = df['content'].apply(lambda x: \
            re.sub(r'(http|ftp|https)://([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])?'
            , '', x))
    df['content'] = df['content'].apply(lambda x: re.sub('RT', '', x))
    df['content'] = df['content'].apply(lambda x: BeautifulSoup(x,
            'lxml').get_text())
...
def load_model_and_tokenizer():
    tokenizer = \
        AutoTokenizer.from_pretrained('rittik9/Pegasus-finetuned-tweet-summary'
            )
    model = \
        AutoModelForSeq2SeqLM.from_pretrained('rittik9/Pegasus-finetuned-tweet-summary'
            ).to(device)
...
choice = st.sidebar.selectbox('Select your choice', ['Summarize Text',
                              'Summarize Twitter Trending Topic'])
...
if choice == 'Summarize Twitter Trending Topic':
    ...
    n = \
        st.text_area('Enter the no. of tweets(1 to 50) you want to summarize'
                     )
...
st.markdown("<h1 style='text-align: center;'>Summarization App By Rittik Panda</h1>"
